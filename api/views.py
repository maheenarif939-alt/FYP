...
def signup(request):
    ...
    user_doc = {
        ...
        'role': 'patient', 'created_at': datetime.datetime.utcnow(),
    }
    ...
    return Response({
        ...
        'tokens': make_tokens(user_doc['_id'], 'patient'),
...
def forgot_password(request):
    ...
    print(f"=== FORGOT PASSWORD REQUEST === email={email} role={role}")

    user = db.users.find_one({'email': email, 'role': role})
    if not user:
        print("=== NO MATCHING USER FOUND — email not sent ===")
        # Security: don't reveal whether this email is registered or not.
        return Response({'message': 'If this email is registered, a reset code has been sent.'}, status=status.HTTP_200_OK)

    print(f"=== USER FOUND: {user.get('full_name')} — sending email now ===")
    ...
    try:
        ...
        print(f"=== EMAIL SENT SUCCESSFULLY to {email}, code={code} ===")
